...
def Interfaccia(ID):                                                                                                    # funzione che mi completa la tebella con le funzioni che ho bisogno
    ...
    choice = input ('Site Visit o Disegno in ufficio? (Site, Design) \n')
    if choice.capitalize() == 'Site':
        design = 'N'
        site_visit = 'Y'
    if choice.capitalize() == 'Design':
        design = 'Y'
        site_visit = 'N'
    if choice.capitalize() != 'Site' and choice.capitalize() != 'Design':
        print('Non ho capito cosa vuoi fare per favore rispondi corretamente alla domanda\n')
        choice = input('Site Visit o Disegno in ufficio? (Site, Design) \n')
        if choice.capitalize() == 'Site':
            design = 'N'
            site_visit = 'Y'
...
def genera_database(file_name, Lista):
    ...
    c.execute("INSERT INTO " + Nome_Table + 'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
              (Lista[0], Lista[1], Lista[2], Lista[3], Lista[4], Lista[5], Lista[6], Lista[7], Lista[8], Lista[9], Lista[10], Lista[11]))
    Data_Base.commit()
    Data_Base.close()
